...
### 🔧 3.2.4 Retry (добавлено)

Реализована повторная отправка batch при ошибках.
...
- retry при timeout
- retry при HTTP 5xx
- НЕ retry при HTTP 4xx

------------------------------------------------------------------------

### 🔧 3.2.5 Split (добавлено)

Реализована изоляция битых событий.

Алгоритм:

- chunk делится пополам
- повторная отправка
- пока не найдётся конкретное событие

------------------------------------------------------------------------

### 🔧 3.2.6 DLQ (добавлено)

Добавлено сохранение ошибок в:

analytics.ingest_dead_letter

------------------------------------------------------------------------

### 🔧 3.2.7 Config (добавлено)

Вынесены все настройки в отдельный модуль:

app/workers/mssql_extractor/config.py
...
- читаются через env
- имеют default значения
- позволяют менять поведение без изменения кода

------------------------------------------------------------------------

### 🔧 3.2.8 Docker env (добавлено)

Настройки прокинуты в docker-compose:
...
Позволяет управлять системой без деплоя.
